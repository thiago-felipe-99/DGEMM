...
void multiplyMatrix(int length, double *matrixA, double *matrixB,
  ...
  int newLength;
  double *A, *B, *C;
  if (length % (AVX_QT_DOUBLE * UNROLL) || (length % BLOCK_SIZE && doBlock)) {
    newLength = length;

    if (newLength % (AVX_QT_DOUBLE * UNROLL))
      newLength +=
          (AVX_QT_DOUBLE * UNROLL - newLength % (AVX_QT_DOUBLE * UNROLL));
    if (newLength % BLOCK_SIZE && doBlock)
      newLength += (BLOCK_SIZE - newLength % BLOCK_SIZE);
