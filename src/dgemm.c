...
void dgemm_simd_manual_unroll(int length, double *a, double *b, double *c) {
  ...
  for (int i = 0; i < length; i++) {
    int j = 0;
    for (j = 0; j < length; j += UNROLL * SIMD_MANUAL_QT_DOUBLE) {
      for (int k = 0; k < length; k++) {
        for (int r = 0; r < UNROLL; r++) {
          c[i * length + j + r + 0] +=
...
void dgemm_avx256_unroll(int length, double *a, double *b, double *c) {
  int i = 0;
  for (; i < length; i += UNROLL * AVX256_QT_DOUBLE) {
    for (int j = 0; j < length; j++) {
      __m256d acc[UNROLL];
      ...
      for (int k = 0; k < length; k++) {
        ...
        for (int r = 0; r < UNROLL; r++) {
          __m256d row = _mm256_load_pd(a + i + k * length + r * AVX256_QT_DOUBLE);
          __m256d mul = _mm256_mul_pd(row, column);
...
void dgemm_avx512_unroll(int length, double *a, double *b, double *c) {
  int i = 0;
  for (; i < length; i += UNROLL * AVX512_QT_DOUBLE) {
    for (int j = 0; j < length; j++) {
      __m512d acc[UNROLL];
      ...
      for (int k = 0; k < length; k++) {
        ...
        for (int r = 0; r < UNROLL; r++) {
          __m512d row = _mm512_load_pd(a + i + k * length + r * AVX512_QT_DOUBLE);
          __m512d mul = _mm512_mul_pd(row, column);
